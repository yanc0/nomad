...
func TestBucket_Iterate(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		err := db.View(func(tx *Tx) error {
			b := tx.Bucket(bucket)
			return b.Iterate(func(key []byte) error {
				var next employee
				if err := b.Get(key, &next); err != nil {
					return err
				}
	...
	t.Run("failure", func(t *testing.T) {
		err := db.View(func(tx *Tx) error {
			b := tx.Bucket(bucket)
			return b.Iterate(func(key []byte) error {
				var i int // not the correct type
				if err := b.Get(key, &i); err != nil {
					return err
				}
				return nil
			})
...
func TestBucket_DeletePrefix(t *testing.T) {
	...
	err := db.View(func(tx *Tx) error {
		bkt := tx.Bucket(bucket)
		return bkt.Iterate(func(k []byte) error {
			var emp employee
			if err := bkt.Get(k, &emp); err != nil {
				return err
			}
