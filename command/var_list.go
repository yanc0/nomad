...
func (c *VarListCommand) Run(args []string) int {
	...
	vars, _, err := client.SecureVariables().List(api.WithNamespace(c.Meta.namespace), api.WithPrefix(prefix))
	if err != nil {
...
func formatVarStubs(vars []*api.SecureVariableListStub) string {
	if len(vars) == 0 {
